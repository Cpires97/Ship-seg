...
def duplicate_bbs(bbs, width, height, img, annot_img, name_image):
    i=1
    cropped_bb=[]
    for final_bb in bbs:
...
#        cropped_img = cropped_img.resize((new_width,new_height))
        ...
        #####
        #cropped_img.save('./airbus-ship-detection/input/train_seg4/cut_images/'+name_image+"_"+str(i)+".png")
        ...
        #cropped_annot.save('./airbus-ship-detection/input/train_seg4/cut_annot/'+name_image+"_"+str(i)+".png")
...
with open('./airbus-ship-detection/input/test_results2/resultskaggle6000_v2/resultkaggle6000_th=0.00001.txt') as json_file:
...
#    result_bbs=[]
    ...
    img=Image.open('./airbus-ship-detection/input/test_results2/test_images/'+image)    
    width, height = img.size    
    name_image, ext=image.split(".")
    annot_img=Image.open('./airbus-ship-detection/input/test_results2/test_labels/'+name_image+'.png')
    ...
    cropped=duplicate_bbs(final, width, height, img, annot_img,name_image)
...
with open('./airbus-ship-detection/input/test_results2/resultskaggle6000_v2/cropped_img_kaggle6000_th=0.00001.txt', 'w') as outfile:
