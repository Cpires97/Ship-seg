...
def duplicate_bbs(bbs, width, height, img, annot_img, name_image):
    ...
    for final_bb in bbs:
       ...
       # cropped_img = cropped_img.resize((480,360))
        ...
        #####
        cropped_img.save('./cut_images/'+name_image+"_"+str(i)+".png")
...
#        cropped_annot = cropped_annot.resize((new_width,new_height)) 
        ...
        ann_img = np.zeros((new_height,new_width)).astype('uint8')
        im = Image.fromarray(ann_img)
        im.save('./cut_annot/'+name_image+"_"+str(i)+".png")
...
with open('./resultkaggle_v2_6000_th=0.0001.txt') as json_file:
...
#    result_bbs=[]
    ...
    aux,aux1, image=filename.split("/")   
    img=Image.open('../test_v2/'+image)    
...
with open('./cropped_img_kaggle_v2_6000_th=0.0001.txt', 'w') as outfile:
